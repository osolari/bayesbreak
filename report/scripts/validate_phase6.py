#!/usr/bin/env python3
"""Independent mechanical validation for the BayesBreak Phase 6 final release."""
...
PHASE_DIR = ROOT / "revision_artifacts" / "phase6"
OUT = PHASE_DIR / "BayesBreak_PHASE_6_VALIDATION.json"
EXPECTATIONS = PHASE_DIR / "RELEASE_EXPECTATIONS.json"
READONLY_HASHES = ROOT / "revision_artifacts" / "phase5" / "PHASE4R_READONLY_HASHES.json"
...
def main() -> None:
    ...
    math_verification = json.loads((PHASE_DIR / "FORMAL_NUMERICAL_VERIFICATION.json").read_text())
    bibliography_verification = json.loads((PHASE_DIR / "BIBLIOGRAPHY_VERIFICATION.json").read_text())
    source_audit = json.loads((PHASE_DIR / "DISTRIBUTABLE_SOURCE_AUDIT.json").read_text())
    handoff_data = json.loads((ROOT / "shared/handoffs/coding_agent_handoff.json").read_text())
    result_data = json.loads((ROOT / "shared/metadata/result_interpretation.json").read_text())
    current_test_state = handoff_status.get("current_bounded_test_state", {})
    historical_test_state = handoff_status.get("historical_bounded_test_state", {})
    corrected_source = "\n".join(
        path.read_text(errors="replace")
        for path in chapter_files + paper_files
    )
    slide_paths = [
    ...
    checks = {
        ...
        "visual_qa_complete": visual_qa.get("status") == "pass"
        and visual_qa.get("total_pages_inspected") == expectations.get("total_pages") == 257
        and visual_qa.get("total_contact_sheets_inspected") == expectations.get("visual_contact_sheets") == 22
        and not visual_qa.get("final_findings", {}).get("reported_defects"),
        "independent_mathematical_checks_pass": math_verification.get("passed") is True
        and len(math_verification.get("checks", [])) == expectations.get("mathematical_checks") == 13
        and all(item.get("passed") for item in math_verification.get("checks", [])),
        "bibliography_verification_report_passes": bibliography_verification.get("passed") is True
        and bibliography_verification.get("counts", {}).get("bibliography_keys") == expectations.get("bibliography_entries") == 38,
        "canonical_registry_counts_match": len(handoff_data.get("tasks", [])) == expectations.get("coding_tasks") == 16
        and len(handoff_data.get("experiments", [])) == expectations.get("experiment_protocols") == 15
        and len(handoff_data.get("claims", [])) == expectations.get("claims") == 14
        and len(handoff_data.get("results", [])) == expectations.get("results") == 17
        and len(result_data.get("results", [])) == expectations.get("results") == 17,
        "historical_and_current_test_states_recorded": all(
            historical_test_state.get(key) == value
            for key, value in expectations.get("historical_test_state", {}).items()
            if key in historical_test_state
        )
        and all(
            current_test_state.get({"timeout_seconds_per_test": "per_test_time_cap_seconds"}.get(key, key)) == value
            for key, value in expectations.get("current_bounded_test_state", {}).items()
            if key not in {"result_id"} or current_test_state.get("result_id") == value
        )
        and historical_test_state.get("collected") == 179
        and historical_test_state.get("passed") == 157
        and historical_test_state.get("skipped") == 5
        and historical_test_state.get("unresolved") == 17
        and current_test_state.get("result_id") == "RES-BB-QA-003"
        and current_test_state.get("collected") == 179
        and current_test_state.get("passed") == 173
        and current_test_state.get("skipped") == 5
        and current_test_state.get("unresolved") == 1
        and current_test_state.get("failed") == 0
        and current_test_state.get("per_test_time_cap_seconds") == 20,
        "phase6_formula_repairs_present": all(token in corrected_source for token in (
            r"m_\star(\theta)=m(\theta;d_\star)",
            r"e^{\Lambda_j}-1",
            r"n_g\log\gamma_u(i,j)",
            r"\min\{1,e^{2\eta}-1\}",
            "maximum cardinality",
        )),
        "distributable_source_audit_passes": source_audit.get("passed") is True
        and source_audit.get("excluded_build_products") is True
        and source_audit.get("font_files_present") == []
        and source_audit.get("cache_artifacts_present") == [],
        ...
        "implementation_test_state_preserved": all(
            ...
            for token in ("RES-BB-QA-002", "RES-BB-QA-003", "173", "157", "17")
        ),
    }
    ...
    validation = {
        ...
        "visual_qa": visual_qa,
        "formal_numerical_verification": math_verification,
        "bibliography_verification": bibliography_verification,
        "distributable_source_audit": source_audit,
        "registry_counts": {
            "coding_tasks": len(handoff_data.get("tasks", [])),
            "experiment_protocols": len(handoff_data.get("experiments", [])),
            "claims": len(handoff_data.get("claims", [])),
            "results": len(handoff_data.get("results", [])),
        },
        "test_states": {
            "historical": historical_test_state,
            "current": current_test_state,
        },
        "real_result_policy": {
