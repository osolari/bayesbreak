...
1. `docs/manuscript/revision_artifacts/adoption/CHATBOT_CHANGE_GUIDE.md`
2. `docs/manuscript/releases/phase6/BayesBreak_PHASE_6_AUTHOR_DECISIONS.md`
3. Canonical sources and registries under `docs/manuscript/`
...
|---|---|---|---|
| Unified Overleaf project | `docs/manuscript/` | Added 319 normalized canonical files | Adopted |
| Outer release package | `docs/manuscript/releases/phase6/` | Added all 23 signed deliverables | Adopted |
| Uppercase annotation key list | `docs/manuscript/releases/phase6/source-collision/ANNOTATION_MANIFEST.json` | Preserved byte-for-byte at a noncolliding path | Adopted |
| Legacy manuscript | former `docs/report/` tree | Removed all 84 superseded files after result verification | Adopted |
...
| Artifact generators | `scripts/`, `src/bayesbreak/experiments/synthetic.py` | Redirected new outputs to `results/`; redundant standalone examples later removed | Adopted |
...
- Release payload: all 22 checksum-manifest entries passed; all 23 outer deliverables are
  retained under `docs/manuscript/releases/phase6/`.
...
- Ruff checks for `src/`, `scripts/`, and `tests/`: passed.
...
| Canonical JSON (`4a778128eb66cd2bce96497e2176ac8e9615d3efbf2e6faf11f97dcdb17c4f97`); renderer (`26723b180386f60d8a483a5d504926beb1ef74706048f0804505fd275e7080cb`) | Detailed TeX appendix, task-registry TeX, and sync manifest (retain synchronized views) | Canonical `CODE-BB-014` and adopted `REV-BB-002`/`REV-BB-012`; preserve stable IDs and one canonical handoff source | `CODE-BB-014` | `docs/manuscript/scripts/check_sync.py` passed: 16 tasks, 15 experiments, 14 claims, 17 results, six failure states, seven stages. | Validated | Generated views remain owned by the canonical JSON renderer; no manual copy was edited. |
