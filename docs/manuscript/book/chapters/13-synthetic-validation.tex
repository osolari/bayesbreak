...
\begin{table}[H]
    \centering
    \input{shared/tables/results/table2_posterior_summary.tex}
...
\begin{table}[H]
    \centering
    \input{shared/tables/results/table3_conjugate_summary.tex}
...
The corrected child \resultid{RES-BB-SYN-005} extends this single archived setting to 400 seeded
datasets over eight predeclared stress cells. The archived-design cell achieved mean hard accuracy
$0.9742$ (95\% interval $0.9536$--$0.9948$) and mean adjusted Rand index $0.9183$. Every returned
objective equaled the final trace value, all traces were monotone, and all 1,200 restarts were valid.
Figure~\ref{fig:latent-stress-corrected} also exposes the expected failure boundary: low separation
and duplicate templates produce weak recovery or collapse, while higher noise and shorter sequences
degrade accuracy. This supports the declared finite score-clustering criterion, not normalized-mixture
identifiability or a global-optimum guarantee.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{shared/figures/results/fig_phase6_latent_stress.png}
    \caption{Corrected latent-group stress execution \resultid{RES-BB-SYN-005}. Top: label-invariant hard accuracy with dataset-level 95\% intervals. Bottom: collapse or duplicate-template rate across the eight declared cells.}
    \label{fig:latent-stress-corrected}
\end{figure}
...
\begin{table}[H]
    \centering
    \input{shared/tables/results/table4_nonconj_tradeoff.tex}
...
\begin{table}[H]
    \centering
    \input{shared/tables/results/table1_runtime_scaling.tex}
...
All populated values and panels in this chapter are real executions. Archived computations remain
unchanged, while \resultid{RES-BB-SYN-005} is a versioned corrected child with independent hashes.
Their role is method validation on the declared synthetic settings, not a comprehensive
state-of-the-art claim.
Boundary calibration reports ECE approximately $0.010$ and Brier approximately $0.011$. The
archived latent-group template experiment reports 96\% hard recovery at $\sigma=1.0$ for 24
sequences; the corrected stress grid reports uncertainty and failure regimes separately.
...
\begin{figure}[H]
\centering
\resizebox{0.96\textwidth}{!}{\input{shared/figures/tikz/experiments/claim_experiment_result_map.tex}}
...
The archive does not yet provide fairness-controlled external comparators across every family,
large-$n$ scaling, or complete misspecification grids. These remain specified as protocols in
Appendix~\ref{app:experiments}. The corrected latent stress cells are populated here; other planned
cells remain explicitly unpopulated.
