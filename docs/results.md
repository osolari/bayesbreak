...
| Region / cell type | $\widehat k$ | Held-out log-predictive |
|---|---:|---:|
| chr21 *methylKit* test region | **15** | **−387.50** |

Held-out evaluation is computed on the last 20% of the chr21 region
under a Beta-observation fit on the first 80%
(`def:metric-loglik`).

## Reproducing these numbers

The values above are stored in
[`docs/report/figures/realdata_metrics.json`](https://github.com/osolari/bayesbreak/blob/master/docs/report/figures/realdata_metrics.json)
and regenerated by
...
[`scripts/figures/`](https://github.com/osolari/bayesbreak/tree/master/scripts/figures).
...
$\widehat k$ when the algorithm requires one. F1@3 is the boundary
F1 at a 3-index tolerance window measured against BayesBreak's MAP
boundaries. Numbers from `docs/report/figures/baselines_metrics.json`.
