...
moment numerator can be zero or negative, implementations use signed-log storage or linear
accumulators with explicit scaling rather than silently taking a logarithm.
...
Memory can be $\Theta(k_{\max}n)$ for basic evidence computation, and can be reduced further by
streaming consecutive $k$-layers when only the final evidence is needed. Boundary posteriors,
Bayes curves, and MAP backtracking require either stored layers/backpointers or deliberate
recomputation, so implementations record which trade-off was used.
