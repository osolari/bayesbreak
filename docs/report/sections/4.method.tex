...
\begin{enumerate}
    ...
    \item[\textnormal{(I2)}] the group templates are pairwise distinct, i.e.\ $(k_g,t^{(g)})\neq(k_{g'},t^{(g')})$ whenever $g\neq g'$;
    \item[\textnormal{(I3)}] the prior on segmentations $p(\tau)$ has positive support on every admissible template, and the block evidences are strictly positive on the support of $y^{(s)}$ for every subject $s$.
\end{enumerate}
Then the joint distribution of $(y^{(1)},\dots,y^{(S)})$ determines the parameter tuple
...
A proof is provided in Appendix~\ref{app:label-switching}. The proposition states what the EM
algorithm can and cannot recover: the equivalence class of $(\pi,\tau_1,\dots,\tau_G)$ modulo label
permutation is identifiable from finite-sample data, but no estimator can distinguish individual
permutations of the latent labels without an external anchoring convention (such as ordering
...
\]
whose log-evidence error is $\varepsilon_Q\downarrow 0$ as $Q\to\infty$ at the rate $O(Q^{-2r})$
for $C^{2r}$ integrands.
\end{proposition}
...
that $|\delta^{\mathrm Q}_{ij}|\le \varepsilon_Q$ uniformly over blocks. The rate is $\varepsilon_Q=O(Q^{-2r})$ when the integrand is $C^{2r}$ on the integration interval.
...
operational meanings: the exported segmentation is a fixed piecewise-constant function
with a discrete change-point structure that is convenient for downstream tasks (e.g.\ region
labeling, hypothesis testing, set-valued prediction); the Bayes curve is a continuous mixture
that integrates segmentation uncertainty and is the appropriate target when smooth predictive
