...
Unless stated otherwise, the standing assumptions are: observations are ordered on a fixed set of candidate boundary locations; the admissible segment-count set is finite; block evidences are finite and positive on admissible blocks and are represented as zero or $-\infty$ on inadmissible blocks; segment parameters are conditionally independent across blocks given a segmentation; and the conditional partition prior factorizes over segments as stated in Assumption~\ref{ass:factorizing-prior}.
...
\begin{enumerate}[label=(\roman*), leftmargin=2.3em]
    ...
    $\arg\max_h p(t_p=h\mid y_{1:n},k)$. These are not generally the same object and must not
    be conflated. The fixed-$k$ MAP objective uses the same length-absorbed block scores as the sum-product DP.
    Count-prior and normalizer offsets such as $\log p(k)-\log C_k$ enter only when we also
    optimize over $k$.
\end{enumerate}
