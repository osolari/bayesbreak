...
Unless stated otherwise, the standing assumptions are: observations are ordered on a fixed
set of candidate boundary locations; the admissible segment-count set is finite; block evidences
are finite and positive on admissible blocks (and zero or $-\infty$ on inadmissible blocks);
segment parameters are conditionally independent across blocks given a segmentation; and the
conditional partition prior factorizes over segments per Assumption~\ref{ass:factorizing-prior}.
...
\begin{enumerate}[label=(\roman*), leftmargin=2.3em]
    ...
    \(p(y_{1:n}\mid k)=\sum_{t\in\mathcal{T}_k}p(t\mid k)\prod_{q}\mathcal{L}(t_{q-1},t_q)\)
    with $\mathcal{T}_k=\{(t_0,\dots,t_k):0=t_0<\cdots<t_k=n\}$;
