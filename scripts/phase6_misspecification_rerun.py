...
def run_shared_cell(seed: int) -> dict[str, Any]:
    ...
    return {
        ...
        "independent_mean_f1": float(np.mean([record["f1"] for record in independent_records])),
...
def summarize(records: list[dict[str, Any]], cell_ids: list[str]) -> dict[str, Any]:
    ...
    for cell_id in cell_ids:
        ...
        if executed and cell_id == "shared-boundary-heterogeneity":
            cell_summary["shared_f1"] = interval_summary(
            ...
            cell_summary["subject_deviation_selected_rate"] = float(
                np.mean(
                    [
                        record["subject_specific_boundary_60_selected_as_shared"]
    ...
    payload: dict[str, Any] = {
        ...
        "resources": {
            ...
            "projected_full_wall_seconds": (
                elapsed * int(plan["full_repetitions_per_cell"])
                if args.mode == "pilot"
                else elapsed
            ),
        },
